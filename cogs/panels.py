...
Panels cog — Embeds interactifs persistants avec boutons et modals.
  ...
  #annonces      → AnnouncePanelView  (info équipe, dispos, aide)
  #roster        → RosterPanelView    (voir roster, rejoindre, profil)
  #calendrier    → CalendarPanelView  (events, ajouter pracc/officiel)
  #praccs        → PraccPanelView     (logger résultat, historique, map stats)
  #mood-overview → MoodPanelView      (set mood, overview — staff)

Commands:
  /panel setup [team]   → installe les panels dans tous les channels
...
from database import (
    ...
    Availability, TeamMood, MoodRating,
)
from utils.channels import get_team_channels
from utils.team_resolver import get_member_role, resolve_team
...
async def _team_from_channel(session, interaction: discord.Interaction) -> Team | None:
    ...
    if not ch_id:
        return None
    tc_r = await session.execute(
        select(TeamChannels).where(
    ...
    if not tc:
        await interaction.response.send_message(
            embed=discord.Embed(
                description="❌  Panel not linked to a team. Run `/panel setup` to recreate it.",
                color=0xBD3944,
            ),
            ephemeral=True,
        )
...
class AddPlayerModal(ui.Modal, title="Add a player"):
    ...
    role = ui.TextInput(label="Role",     placeholder="IGL, Duelist, Initiator, Sentinel, Controller", required=False, max_length=30)
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        async for session in get_session():
            existing_r = await session.execute(
                select(Player).where(Player.discord_id == interaction.user.id, Player.team_id == self.team.id)
            )
            if existing_r.scalar_one_or_none():
                await interaction.followup.send(
                    embed=discord.Embed(description=f"You're already in **{self.team.name}**'s roster.", color=0xF0A500),
                    ephemeral=True,
                )
                return
            player = Player(
            ...
            from utils.channels import auto_assign_role
            ...
            if interaction.guild:
                await post_roster_update(interaction.guild, session, self.team, "added", self.ign.value, self.role.value or None, interaction.user)
            await interaction.followup.send(
                embed=discord.Embed(description=f"**{self.ign.value}#{self.tag.value}** added to **{self.team.name}**!", color=0x00C17C),
                ephemeral=True,
            )
...
class LogPraccModal(ui.Modal, title="Log a Pracc"):
    ...
    score      = ui.TextInput(label="Score (W-L)", placeholder="13-7",                required=False, max_length=10)
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        if self.score.value.strip():
            try:
                parts = self.score.value.strip().replace(" ", "").split("-")
                if len(parts) == 2:
                    rounds_won, rounds_lost = int(parts[0]), int(parts[1])
        ...
        await interaction.followup.send(
            embed=discord.Embed(description=f"Pracc vs **{self.opponent.value}** logged! {score_display}", color=0x00C17C),
            ephemeral=True,
        )
...
class AddEventModal(ui.Modal, title="Add an Event"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        await interaction.response.defer(ephemeral=True)
        try:
            scheduled_at = datetime.strptime(f"{self.date_input.value.strip()} {self.time_input.value.strip()}", "%d/%m/%Y %H:%M")
        except ValueError:
            await interaction.followup.send(
                embed=discord.Embed(description="Invalid format. Use DD/MM/YYYY and HH:MM.", color=0xBD3944),
                ephemeral=True,
            )
            return
        async for session in get_session():
        ...
        await interaction.followup.send(
            embed=discord.Embed(description=f"**{self.title_input.value}** added to the calendar!", color=0x00C17C),
            ephemeral=True,
        )


class MoodModal(ui.Modal, title="My Mood This Week"):
    rating = ui.TextInput(label="Rating (1 to 5)", placeholder="4", max_length=1)
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        if val not in ("1", "2", "3", "4", "5"):
            await interaction.followup.send(
                embed=discord.Embed(description="Rating must be 1, 2, 3, 4 or 5.", color=0xBD3944),
                ephemeral=True,
            )
            return
        async for session in get_session():
            ...
            existing_r = await session.execute(
                select(TeamMood).where(TeamMood.player_id == player.id, TeamMood.week_start == week_start)
            )
            ...
            if existing:
                ...
                session.add(TeamMood(player_id=player.id, team_id=team.id, rating=MoodRating(val), note=self.note.value or None, week_start=week_start))
            await session.commit()
            from utils.poster import post_mood_overview
            if interaction.guild:
                await post_mood_overview(interaction.guild, session, team, week_start)
            EMOJI = {"1": "😞", "2": "😕", "3": "😐", "4": "🙂", "5": "😄"}
            await interaction.followup.send(
                embed=discord.Embed(description=f"Mood **{val}/5** {EMOJI.get(val, '')} saved!", color=0x00C17C),
                ephemeral=True,
            )
...
class AnnouncePanelView(ui.View):
    ...
    async def team_info(self, interaction: discord.Interaction, button: ui.Button) -> None:
        ...
        async for session in get_session():
            ...
            if not team:
                return
            players_r = await session.execute(select(Player).where(Player.team_id == team.id, Player.is_active == True))
            members_r = await session.execute(select(TeamMember).where(TeamMember.team_id == team.id))
            players = players_r.scalars().all()
            members = members_r.scalars().all()
            embed = discord.Embed(title=f"ℹ️  {team.name}" + (f" [{team.tag}]" if team.tag else ""), color=0xFF4655)
            ...
            embed.add_field(name="📋 Commands", value="`/roster` `/dispo` `/cal` `/pracc` `/mapstats` `/mood` `/stats`", inline=False)
    ...
    async def my_dispo(self, interaction: discord.Interaction, button: ui.Button) -> None:
        ...
        async for session in get_session():
            ...
            embed = discord.Embed(title="📆  My Availability — This Week", color=0xFF4655)
            if not avails:
    ...
    async def help_cmd(self, interaction: discord.Interaction, button: ui.Button) -> None:
        await interaction.response.defer(ephemeral=True)
        embed = discord.Embed(title=f"📖  {config.bot_name} — All Commands", color=0xFF4655)
        ...
        embed.add_field(name="⚙️ Staff",          value="`/team invite` `/team kick` `/panel setup` `/panel refresh`", inline=False)
        embed.set_footer(text=config.bot_name)
        await interaction.followup.send(embed=embed, ephemeral=True)
...
class RosterPanelView(ui.View):
    ...
    async def view_roster(self, interaction: discord.Interaction, button: ui.Button) -> None:
        ...
        async for session in get_session():
            ...
            if not players:
                ...
                lines = [f"{'`' + p.role + '`  ' if p.role else ''}**{p.ign}**#{p.tag or '???'}" for p in players]
                embed.description = "\n".join(lines)
    ...
    async def my_profile(self, interaction: discord.Interaction, button: ui.Button) -> None:
        ...
        async for session in get_session():
            ...
            if not team:
                return
            player_r = await session.execute(select(Player).where(Player.discord_id == interaction.user.id, Player.team_id == team.id))
            player = player_r.scalar_one_or_none()
            if not player:
                await interaction.followup.send(
                    embed=discord.Embed(description="You're not in this team's roster.", color=0xBD3944),
    ...
    async def view_events(self, interaction: discord.Interaction, button: ui.Button) -> None:
        ...
        async for session in get_session():
            ...
            events_r = await session.execute(
                select(Event).where(Event.team_id == team.id, Event.scheduled_at >= datetime.utcnow())
                .order_by(Event.scheduled_at).limit(10)
            )
            events = events_r.scalars().all()
            icons = {"pracc": "🥊", "official": "🏆", "meeting": "📋"}
            ...
            if not events:
                ...
                lines = []
                for ev in events:
                    icon = icons.get(ev.event_type.value, "📅")
                    opp  = f" vs **{ev.opponent}**" if ev.opponent else ""
                    ts   = int(ev.scheduled_at.timestamp())
                    lines.append(f"{icon} <t:{ts}:d> <t:{ts}:t>{opp}")
                embed.description = "\n".join(lines)
            await interaction.followup.send(embed=embed, ephemeral=True)
    ...
    async def add_pracc(self, interaction: discord.Interaction, button: ui.Button) -> None:
        async for session in get_session():
            ...
            if not _is_staff(interaction, role):
                await interaction.response.send_message(embed=discord.Embed(description="Staff only.", color=0xBD3944), ephemeral=True)
                return
    ...
    async def history(self, interaction: discord.Interaction, button: ui.Button) -> None:
        ...
        async for session in get_session():
            ...
            if not team:
                return
            events_r = await session.execute(
                select(Event).where(Event.team_id == team.id, Event.event_type == EventType.PRACC)
                .order_by(Event.scheduled_at.desc()).limit(10)
            )
            events = events_r.scalars().all()
            icons = {MatchResult.WIN: "🟢", MatchResult.LOSS: "🔴", MatchResult.DRAW: "🟡", MatchResult.PENDING: "⏳"}
            ...
            if not events:
                ...
                lines = []
                for ev in events:
                    icon  = icons.get(ev.result, "⏳")
                    score = f" `{ev.rounds_won}-{ev.rounds_lost}`" if ev.rounds_won is not None else ""
                    map_t = f" · {ev.map_played}" if ev.map_played else ""
                    lines.append(f"{icon} vs **{ev.opponent or '???'}**{score}{map_t}")
                embed.description = "\n".join(lines)
            await interaction.followup.send(embed=embed, ephemeral=True)
    ...
    async def map_stats(self, interaction: discord.Interaction, button: ui.Button) -> None:
        ...
        async for session in get_session():
            ...
            if not team:
                return
            events_r = await session.execute(
                select(Event).where(Event.team_id == team.id, Event.result != MatchResult.PENDING, Event.map_played != None)
            )
            events = events_r.scalars().all()
            if not events:
                await interaction.followup.send(embed=discord.Embed(description="No map data yet.", color=0xFF4655), ephemeral=True)
                return
    ...
    async def overview(self, interaction: discord.Interaction, button: ui.Button) -> None:
        ...
        async for session in get_session():
            ...
            moods_r = await session.execute(
                select(TeamMood).where(TeamMood.team_id == team.id, TeamMood.week_start == week_start)
            )
            moods = moods_r.scalars().all()
            EMOJI = {"1": "😞", "2": "😕", "3": "😐", "4": "🙂", "5": "😄"}
            embed = discord.Embed(title=f"💬  Mood — Week of {week_start.strftime('%d/%m/%Y')}", color=0xFF4655)
            if not moods:
                ...
                for m in moods:
                    ...
                    if p:
                        ...
                        embed.add_field(name=f"{EMOJI.get(m.rating.value, '😐')} {p.ign}", value=f"`{m.rating.value}/5`{note}", inline=True)
            await interaction.followup.send(embed=embed, ephemeral=True)
...
class PanelsCog(commands.Cog, name="Panels"):
    ...
    @panel.command(name="setup", description="Install interactive panels in all team channels")
    @app_commands.describe(team_name="Team (optional)")
    @app_commands.autocomplete(team_name=team_autocomplete)
    async def panel_setup(self, interaction: discord.Interaction, team_name: Optional[str] = None) -> None:
        await interaction.response.defer(ephemeral=True)
        async for session in get_session():
            ...
            if not tc:
                await interaction.followup.send(
                    embed=discord.Embed(description="No channels found. Run `/team setup-channels` first.", color=0xBD3944),
                    ephemeral=True,
                )
                return
            panels = [
                (tc.ch_announcements, "panel_announce_msg",  _announce_panel_embed(team),  AnnouncePanelView()),
                (tc.ch_roster,        "panel_roster_msg",    _roster_panel_embed(team),    RosterPanelView()),
                (tc.ch_calendar,      "panel_calendar_msg",  _calendar_panel_embed(team),  CalendarPanelView()),
                (tc.ch_praccs,        "panel_praccs_msg",    _pracc_panel_embed(team),     PraccPanelView()),
                (tc.ch_mood,          "panel_mood_msg",      _mood_panel_embed(team),      MoodPanelView()),
            ]
            created = errors = 0
            for ch_id, msg_field, embed, view in panels:
                ...
                try:
                    ...
                    errors += 1
            await session.commit()
            status = f"✅  **{created}/5** panel(s) installed"
            if errors:
    ...
    @panel.command(name="refresh", description="Refresh all panel embeds")
    @app_commands.describe(team_name="Team (optional)")
    @app_commands.autocomplete(team_name=team_autocomplete)
    async def panel_refresh(self, interaction: discord.Interaction, team_name: Optional[str] = None) -> None:
        await interaction.response.defer(ephemeral=True)
        async for session in get_session():
            team = await resolve_team(
                session, interaction.guild_id, interaction.user.id,
            ...
            for ch_id, msg_id, embed, view in panels:
                ...
                try:
                    ...
                    pass
            await interaction.followup.send(
                embed=discord.Embed(description=f"✅  **{refreshed}** panel(s) refreshed.", color=0x00C17C),
                ephemeral=True,
            )
